...
export class MyBookingsComponent implements OnInit {
    ...
    this.bookingService.getUserBookings().subscribe({
      next: (response: any) => {
        ...
        if (response.success) {
          ...
          console.log('Customer bookings loaded:', this.bookings);
        } else {
          this.toastService.showError('Failed to load bookings');
          console.error('Failed to load customer bookings:', response);
        }
      },
      error: (error: any) => {
        this.isLoading = false;
  ...
  onImageError(event: any): void {
    ...
    if (target && !target.src.endsWith('default-car.jpg')) {
